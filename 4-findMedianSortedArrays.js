...
//方法一、 合并新数组排序取中位数，时间复杂度为O((m+n)log(m+n))不符合
...
//     } else {
//         //偶数位取中间两个值平均值
//         const midNums = (newArr[newArr.length / 2] + newArr[(newArr.length / 2) - 1]) / 2;
//         return midNums;
//     }
// };
...
var findMedianSortedArrays = function (nums1, nums2) {
    ...
    let totalLeft = Math.floor((leftLength + rightLength + 1) / 2);
    // 在 nums1 的区间 [0, leftLength] 里查找恰当的分割线，
    // 使得 nums1[i - 1] <= nums2[j] && nums2[j - 1] <= nums1[i]
    let left = 0;
    let right = leftLength;
    // nums1[i - 1] <= nums2[j]
...
const nums1 = [3];
const nums2 = [-2, -1];
const midNums = findMedianSortedArrays(nums1, nums2);
